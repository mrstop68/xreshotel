...
<body>
    ...
    <header id="header" class="header d-flex align-items-center fixed-top">
      ...
      <div class="container-fluid container-xl d-flex align-items-center justify-content-between">
        <a class="logo d-flex align-items-center" href="<?php if(empty($langURL)){echo $dataHOTEL->website;}else{ echo $dataHOTEL->website.'/'.$langURL.'/';} ?>"><img class="logo" src="<?=$imagesLink?>logo/<?php if(isset($dataHOTEL->logo->logoname)) echo $dataHOTEL->logo->logoname; ?>" alt="<?=$seoData->imagetag?>"></a>
                  ...
                  <?php if(isset($dataLANG[1]->LangCode)){ //dil menu start?> 
                      <li class="dropdown">
                        <?php if(empty($langURL)){ // ana header üzerinde görünecek dil?>
                            <a class="nav-tab"><span><img class="flags" src="<?=$imagesLink?><?=$dataHOTEL->LangLink?>" alt="<?=$seoData->imagetag?>" width="26"> <?=$dataHOTEL->LangCode?></span><i class="bi bi-chevron-down dropdown-indicator"></i></a>
                        <?php }else { ?>
                            <a class="nav-tab"><span><img class="flags"src="<?=$imagesLink?>img/flag/<?=strtoupper($langURL)?>.svg" alt="<?=$seoData->imagetag?>" width="26"> <?=strtoupper($langURL)?></span><i class="bi bi-chevron-down dropdown-indicator"></i></a>
                        <?php }  ?>
                        <ul>
                            <?php  
                                    if(empty($langURL)){ // header üzerindeki dili tıklayınca listelenen diller
                                        ...
                                        <?php } } ?>
                                    <?php }else{  ?>
                                        <li class="flags-li"> <a href="<?=$dataHOTEL->website?>/" ><img class="flags" src="<?=$imagesLink?><?=$dataHOTEL->LangLink?>" alt="<?=$seoData->imagetag?>" width="26"> <?=$dataHOTEL->LangCode?></a></li>
