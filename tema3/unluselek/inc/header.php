    ...
    <section class="theadert">
        <div class="top-header">
            <div class="containerhead">
                <div>
                    <ul class="top-menu">
                        <li><a href="tel:+<?=$dataHOTEL->seoinfo->phone1?>"><img src="/tema3/images/headphones.svg" alt="<?=$seoData->imagetag?>"> <?=$dataHOTEL->seoinfo->phone1?> </a></li>
                        <?php if(isset($dataLANG[1]->LangCode)){ //dil menu start?> 
                        <li>
                        <?php if(empty($langURL)){ // ana header üzerinde görünecek dil?>
                            <img class="flags" src="<?=$imagesLink?><?=$dataHOTEL->LangLink?>" alt="<?=$seoData->imagetag?>" width="26"> <?=$dataHOTEL->LangCode?> <span> <img src="/tema3/images/angle-down-solid.svg" alt="<?=$seoData->imagetag?>" width="8px" height="13"></span>
                            <?php }else { ?>
                                <img class="flags" src="<?=$imagesLink?>img/flag/<?=strtoupper($langURL)?>.svg" alt="<?=$seoData->imagetag?>" width="26"> <?=strtoupper($langURL)?> <span> <img src="/tema3/images/angle-down-solid.svg" alt="<?=$seoData->imagetag?>" width="8px" height="13"></span>
                            <?php }  ?>
                            ...
                            <?php  
                                    if(empty($langURL)){ // header üzerindeki dili tıklayınca listelenen diller
                                        ...
                                        <?php if(($langs->LangCode)!='mainlang'){ ?>
                                                <li><a href="/<?=strtolower($langs->LangCode)?>/"> <img class="flags" src="<?=$imagesLink?><?=$langs->LangLink?>" alt="<?=$seoData->imagetag?>" width="26"> <?=$langs->LangCode?></a></li>
                                         <?php } } ?>
                                    <?php }else{  ?>
                                                <li><a href="/"> <img class="flags" src="<?=$imagesLink?><?=$dataHOTEL->LangLink?>" alt="<?=$seoData->imagetag?>" width="26"> <?=$dataHOTEL->LangCode?></a></li>
        ...
        <header id="navbar">
            <div class="containerhead">
                <div class="header-logo">
                    <a href="<?php if(empty($langURL)){echo '/';}else{ echo '/'.$langURL.'/';} ?>">
                    ...
                    if((empty($langURL))){
                        ...
                        foreach($filterPageToLang as $pages){
                            if($pages->status!='false'){ ?>
                                <li><a href="<?php if($pages->link=='/'){echo'/';}else{echo $pages->link;} ?>" <?php if((isset($pages->link)) && ($pages->link=='reservation'))echo 'id="tbres"' ?>><?=$pages->pagename?></a></li>
                        ...
                        foreach($filterPageToLang as $pages){
                            if($pages->status!='false'){ ?>
                                <li><a href="<?php if($pages->link=='/'){echo '/'.$langURL.'/';}else{echo $pages->link;} ?>" <?php if((isset($pages->link)) && ($pages->link=='reservation'))echo 'id="tbres"' ?>><?=$pages->pagename?></a></li>
